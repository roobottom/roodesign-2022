 ...
 * @param {String} [layout] - One of the following [full, right, left, text], optional, defaults to `text`
 */
module.exports = function(images, caption, style, layout) {

  let styleClass = style ? `gallery--${style}` : ''
  let layoutClass = layout ? `gallery--${layout}` : 'gallery--text'

  let alignSingleImageRight = layout === 'right' ? 'max-bleed-left-s' : ''
  let alignSingleImageWithText = layout === 'text' ? 'max-bleed-right-xl' : ''
  let wrapperClassNames = images.length > 1 ? `gallery-images list list--grid-gallery` : `gallery-single-image ${alignSingleImageRight} ${alignSingleImageWithText}`

  const imagesHtml = (images) => {
    let html = ''
    for (let image of images) {
      html = html + img(image.url, null, image.alt)
    }
    return html
  }
